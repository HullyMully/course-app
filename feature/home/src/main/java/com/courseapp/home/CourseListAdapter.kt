...
private fun resolveLocalImage(title: String, resources: android.content.res.Resources, packageName: String): Int {
    val name = when {
    ...
    adapterDelegateViewBinding<CourseItem, CourseItem, ItemCourseBinding>(
        ...
        bind {
            ...
            if (!imageUrl.isNullOrBlank()) {
                ...
                val localRes = resolveLocalImage(item.dto.title, context.resources, context.packageName)
                if (localRes != 0) {
                    Glide.with(binding.image)
                        .load(localRes)
                        .transform(CenterCrop())
